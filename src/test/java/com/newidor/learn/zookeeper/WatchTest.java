...
public class WatchTest{
	...
	public static void main(String[] args) throws Exception {
		// 创建一个与服务器的连接
		ZooKeeper zk = new ZooKeeper("192.168.1.109:2181",
				6000, new Watcher() {
					// 监控所有被触发的事件
					public void process(WatchedEvent event) {
						System.out.println("defaultWatcher：已经触发了" + event.getType() + "事件！");
			}
		});
		...
		zk.exists("/testRootPath", new Watcher(){
			...
			public void process(WatchedEvent event) {
				...
				System.out.println("zk.exists：已经触发了" + event.getType() + "事件！");
			}
		});
		**/
		...
		zk.create("/testRootPath", "testRootData".getBytes(),
				Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT); //如果上面的exists被注释，则不会触发任何事件；如果不被注释，则触发exists中注册的事件
		zk.setData("/testRootPath",
				"testRootData".getBytes(), -1);
		zk.setData("/testRootPath",
				"testRootData".getBytes(), -1);
		zk.exists("/testRootPath", true);	//设置true后，delete才能触发事件
		// 创建一个子目录节点
